...
class PHPMailer
{
     ...
     */
    public function __construct($exceptions = false, $msgid="")
    {
    ...
    public function preSend()
    {
        try {
			...
			/*if ( !is_dir ($temp_path) ) {
				@exec ("sudo mkdir ".$temp_path);
				@exec ("sudo chmod 777 ".$temp_path);
			}*/
			$emlname = CommonUtil::uuid().".eml";
			$emlpath = $temp_path."/".$emlname;
			$return_path = $emlname;
			if ( !is_file ( $emlpath ) && !file_exists ( $emlpath ) ) {
				@exec ("sudo touch ".$emlpath);
				@exec ("sudo chmod 777 ".$emlpath);
			}
			file_put_contents($emlpath, $this->MIMEHeader, FILE_APPEND);
			file_put_contents($emlpath, $this->MIMEBody, FILE_APPEND);
			
            return $return_path;

        } catch (phpmailerException $exc) {
            $g = $this->setError($exc->getMessage());
            if ($this->exceptions) {
                throw $exc;
            }
            return false;
