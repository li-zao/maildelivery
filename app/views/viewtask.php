...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"><a href="/createtem/firstpage" title="返回首页" class="tip-bottom"><i class="icon-home"></i> 首页</a><a href="#"> 投递任务管理</a><a href="#" class="current">查看任务</a></div>
  </div>
							 ...
							 <tbody>
								...
								<tr>
									<td colspan="2" style="text-align: left;padding: 0;border:0;padding-top:0;" >
										<input type="text" id="taskname" name="task_name" value="{%$task_name%}" onblur="" style="border: 1px solid rgb(191, 191, 191); line-height: 25px; width: 99.3%; padding: 0px 0px 0px 4px;  margin-bottom: 0px; height: 25px;font-size:12px;">
							...
							<input aria-hidden="true" data-dismiss="modal" type="button" class="btn" id="slelectconfirm" value="确定" />
										...
										<div id="appends" style="margin: 3px;">
											{%section name=loop loop=$attachmentdata%}
												<span class="see" id="{%$attachmentdata[loop].path%}" style="margin-left: 10px;"><label id="{%$attachmentdata[loop].truename%}" for="">{%$attachmentdata[loop].truename%}</label><a href="javascript:void(0)" id="{%$attachmentdata[loop].id%}" onclick="delattch(this)"><i class="icon-remove" style="margin-left: 10px;"></i></a></span>
											{%/section%}
										</div>
										{%else%}	
				...
				$('#attachs').modal('show');
								...
								<tr>
									<td colspan="4" style="text-align: left;padding: 10px 10px 0px 90px;border:0;line-height:20px;font-size: 12px;">
										...
										<!--定时发送-->
										<div id="timer_send" style="line-height: 20px; border: 2px dotted rgb(204, 204, 204); width: 76%; padding-top: 10px;padding-bottom: 10px; padding-left: 10px; display: block;">请设置发送时间：
										{%if $sendtype == 1%}
										<div class="input-append date form_datetime" style="margin-right: 1px;margin-top: -4px;"><input size="16" name="start_time" id="start_time" type="text" value="{%$sendtime%}" style="width:200px;" readonly><span class="add-on"><i class="icon-th"></i></span></div><br/>定时计划会在您设定的时间开始尝试发送邮件。在此之前，您还可以撤销发送计划对邮件进行编辑。
										{%else%}
										<div class="input-append date form_datetime" style="margin-right: 1px;margin-top: -4px;"><input size="16" name="start_time" id="start_time" type="text" value="" style="width:196px;" readonly><span class="add-on" style="padding-bottom: 0px\9;height:19px\9;"><i class="icon-th"></i></span></div><br/>定时计划会在您设定的时间开始尝试发送邮件。在此之前，您还可以撤销发送计划对邮件进行编辑。
										{%/if%}
										</div>
										<!--周期发送-->
										...
										发送时间：<select id="cycle_time" name="cycle_time"  style="padding: 0px; width: 65px; height: 20px; margin-bottom: 5px;">				
												...
												{%if $cycle_end_time == '0000-00-00' || $cycle_end_time == null%}
													<input size="16" name="next_end_time" id="next_end_time" type="text" value="" style="width:196px;" readonly><span class="add-on" style="padding-bottom: 0px\9;height:19px\9;"><i class="icon-th"></i></span>
												{%else%}
													<input size="16" name="next_end_time" id="next_end_time" type="text" value="{%$cycle_end_time%}" style="width:200px;" readonly><span class="add-on"><i class="icon-th"></i></span>
												{%/if%}
												</div>
												<table width="100%">
													<tbody><tr>
								...
								<tr>
									<td colspan="4" style="text-align: left;padding: 20px 0px 8px 15px;border:0;">
										<button id="back_btn" class="btn" type="button" style="width:85px;height:27px;background:url(/dist/img/but.jpg) no-repeat;border-radius: 3px 3px 3px 3px;margin-right: 5px;">返回</button>
										{%if $mine == '1' %}
										<button id="copy_btn" class="btn" type="button" style="width:85px;height:27px;background:url(/dist/img/but.jpg) no-repeat;border-radius: 3px 3px 3px 3px;margin-right: 5px;">另存</button>
										{%/if%}
									</td>
								</tr>
							</tbody>
