...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"><a href="/createtem/firstpage" title="返回首页" class="tip-bottom"><i class="icon-home"></i> 首页</a><a href="#"> 统计分析</a><a href="#" class="current">按发布人员统计</a></div>
  </div>
 		...
 		<div class="row-fluid">
			<div class="span12">
				<div class="widget-box" style="margin-bottom: 10px;">
					 ...
					 <form method="get" action="/statistics/releaseperson" id="general_from">
						<table style="height: 90px;width:100%; margin-top: 10px; margin-bottom: 10px;">
							<tr>
								<td style="padding-left: 42px;width:120px;">选择发布人员统计：</td>
								<td style="width:16%">
									<select name="userid" style="width: 173px;">
									<option value="0">请选择发布人员</option>
									{%section name=loop loop=$userdata%}
										<option value="{%$userdata[loop].id%}" {%if $userid == $userdata[loop].id%}selected="selected"{%/if%}>{%$userdata[loop].username%}</option>
									{%/section%}			
									</select>
								</td>
							...
							<tr>
								<td colspan="4" style="padding-left: 42px;">
									...
									<!--<label class="radio" style="width:195px"><input type="radio" name="statisticstime" id="" style="margin-left: 0px; margin-right: 10px;" value="4"  {%if $statisticstime == 4%}checked{%/if%}>按季统计</label>-->
							...
							<tr><td colspan="4" style="text-align:center;height:30px"><button id="begin" class="btn" type="button" style="padding-left: 20px; padding-right: 20px;">开始统计</button></td></tr>
						</table>
					</form>
					 </div>
				</div>
								...
								{%if $infos%}
									{%foreach item=temp from=$infos%}
										<tr>
											...
											<td class="taskStatus"><span class="in-progress">{%if $temp.open != ""%}{%$temp.done%}{%else%}0{%/if%}</span></td>	
											<td class="taskStatus"><span class="in-progress">{%if $temp.skip != ""%}{%$temp.skip%}{%else%}0{%/if%}</span></td>	
											<td class="taskStatus"><span class="in-progress">{%$temp.unsubscribe%}{%if $temp.unsubscribe != ""%}{%$temp.unsubscribe%}{%else%}0{%/if%}</span></td>	
											<!--<td class="taskStatus"><span class="in-progress">{%$temp.trashmail%}{%if $temp.trashmail != ""%}{%$temp.trashmail%}{%else%}0{%/if%}</span></td>-->
										</tr>
									{%/foreach%}
								{%/if%}
								</tbody>
