	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			changeSort: function(th) { //change sortorder
				...
				$('.ssdesc',this.hDiv).removeClass('ssdesc');
				$('.ssasc',this.hDiv).removeClass('ssasc');
				$('div',th).addClass('ss'+p.sortorder);
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									if (!isNaN(parseInt($(g.nBtn).css('borderLeftWidth')))) {
										nw = nw + parseInt($(g.nBtn).css('borderLeftWidth'));
									}
									nl = onl - nw + Math.floor(p.cgwidth/2);
									$(g.nDiv).hide();
									$(g.nBtn).hide();
									$(g.nBtn).css({'top':g.hDiv.offsetTop}).show();
